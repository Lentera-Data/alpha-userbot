    ...
    if G_DRIVE_DATA is not None:
        ...
        if G_DRIVE_CLIENT_ID is None and G_DRIVE_CLIENT_SECRET is None:
            await gdrive.edit(
                "`[AUTHENTICATE - ERROR]`\n\n"
                "`Status`: **BAD**\n"
                "`Reason`: Please get your **G_DRIVE_DATA** variable "
...
async def download_gdrive(gdrive, service, uri):
    ...
    try:
        ...
        if mimeType == 'application/vnd.google-apps.folder':
            await gdrive.edit("`Aborting, folder download not supported...`")
            return False
        file_path = TEMP_DOWNLOAD_DIRECTORY + file_name
...
async def create_dir(service, folder_name):
    ...
    folder = service.files().create(
           body=metadata, fields="id, webViewLink", supportsAllDrives=True).execute()
    ...
    if isfile(value):
        ...
        if not uri and not gdrive.reply_to_msg_id:
            await gdrive.edit(
                ...
                "to upload from files/folders path. This is unsupported."
            )
            return False
...
async def set_upload_folder(gdrive):
    ...
    if not inp:
        await gdrive.edit("Usage: `.gdfset put <folderURL/folderID>`")
        return None
